...
void neverlose::panic(const char* fmt, ...)
{
	...
	va_list va;
	va_start(va, fmt);
	vsnprintf_s(buffer, sizeof(buffer), _TRUNCATE, fmt, va);
	va_end(va);
...
void neverlose::map(HMODULE hModule)
{
	...
	const DWORD Size = SizeofResource(hThis, hRes);
	if (!Size)
		panic("Cheat binary resource is empty!");
...
PVOID neverlose::load_res_to_mem(int idr, const char* rcname) const
{
	...
	const DWORD Size = SizeofResource(hThis, hRes);
	if (!Size)
		panic("%s binary is empty!", rcname);
